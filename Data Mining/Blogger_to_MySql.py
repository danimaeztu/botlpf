...
service = build('blogger', 'v3', developerKey=cf.developerKey)
posts = service.posts()

# Mysql connection and create schema and tables
mysql = "mysql://{}:{}@{}".format(
        cf.sql_user, cf.sql_pw, cf.sql_host)
engine = sqlalchemy.create_engine(mysql)
connection = engine.connect()

with open('{}\\create_tables.sql'.format(cf.path), 'r') as file:
    sql = file.read()
...
while pageToken is not False:
    data = posts.list(blogId=cf.blogId, pageToken=pageToken).execute()
...
twitterusers = pd.read_csv('{}\\csv\\Twitter_users.csv'.format(cf.path),
                           encoding='latin_1')
